...
int main(void)
{
	...
	ASSERT(stat("listing_0054_draw_rectangle", &disassembly_stat) == 0);

	unsigned long long disassembly_size = disassembly_stat.st_size;
	s32 disassembly_fd = open("listing_0054_draw_rectangle", O_RDONLY);
	...
	s32 output_image_fd = open("output_image.data", O_CREAT | O_RDWR);
	s32 bytes_written = write(output_image_fd, memory, 65536);
