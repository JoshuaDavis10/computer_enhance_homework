...
u32 get_ea_clocks(u32 reg_one, u32 reg_two, s32 disp)
{
	/* NOTE: I'm assuming Casey's decoder is good enough that we'll never end up with
	 * registers other than BX, BP, SI, DI in here
	 */

	/* disp only */
	if(reg_one == 0 && reg_two == 0)
	{
		return 6;
	}
	/* base/index only */
	...
	{
		/* BP + DI or BX + SI */
		if((reg_one == 6 && reg_two == 8) || (reg_one == 2 && reg_two == 7))
		{
			return 7;
		}
		else
	...
	/* if we've gotten this far, then they're all non-zero*/
	...
	if(op_type == Op_mov)
	{
		...
		if(op_one_type == Operand_Register)
		{
			if(op_two_type == Operand_Register)
			{
				...
				print_reg(reg_two);
				printf(" ; ");
				/* register, register mov is 2 clocks, according to manual */
				clocks += 2;
				printf("Clocks +2 = %u | ", clocks);
				print_reg(reg_one);
			...
			else if(op_two_type == Operand_Immediate)
			{
				...
				print_reg(reg);
				printf(", %d ; ", imm);
				/* register, immediate mov is 4 clocks, according to manual */
				clocks += 4;
				printf("Clocks +4 = %u | ", clocks);
				print_reg(reg);
			...
			else if(op_two_type == Operand_Memory)
			{
				...
				u32 temp_ea_clocks = 
					get_ea_clocks(
						...
						address_calc_reg_two_value, 
						address_calc_displacement);
				/* register, memory mov is 8 clocks + EA, according to manual */
				clocks += 8 + temp_ea_clocks;
				printf("Clocks +%u = %u | ", (8 + temp_ea_clocks), clocks);
				print_reg(reg);
				printf(":0x%x->0x%x ", temp, registers[reg - 1]);
			}
			else
		...
		if(op_one_type == Operand_Memory)
		{
			if(op_two_type == Operand_Immediate)
			{
				...
				memory[address_calc+1] = (u8)high_bits >> 8;
				memory[address_calc] = (u8)low_bits;
				...
				u32 temp_ea_clocks = 
					get_ea_clocks(
						...
						address_calc_reg_two_value, 
						address_calc_displacement);
				/* memory, immediate mov is 10 clocks + EA, according to manual */
				clocks += 10 + temp_ea_clocks;
				printf("Clocks +%u = %u | ", (10 + temp_ea_clocks), clocks);
			}
			else if(op_two_type == Operand_Register)
			{
				u16 address_calc_reg_one_value;
				u16 address_calc_reg_two_value;
				u32 address_calc_reg_one_index = execute.Operands[0].Address.Terms[0].Register.Index;
				...
				u32 reg = execute.Operands[1].Register.Index;
				u16 high_bits = ((registers[reg - 1]) & 0b1111111100000000);
				u16 low_bits  = ((registers[reg - 1]) & 0b0000000011111111);
				memory[address_calc+1] = (u8)high_bits >> 8;
				memory[address_calc] = (u8)low_bits;
				...
				u32 temp_ea_clocks = 
					get_ea_clocks(
						...
						address_calc_reg_two_value, 
						address_calc_displacement);
				/* memory, register mov is 9 clocks + EA, according to manual */
				clocks += 9 + temp_ea_clocks;
				printf("Clocks +%u = %u | ", (9 + temp_ea_clocks), clocks);
			}
			else
			{
	...
	else if(op_type == Op_add)
	{
		...
		if(op_one_type != Operand_Register)
		{
			if(op_one_type == Operand_Memory)
			{
				...
				high_bits = (temp & 0b1111111100000000);
				low_bits  = (temp & 0b0000000011111111);
				memory[address_calc+1] = (u8)high_bits >> 8;
				memory[address_calc] = (u8)low_bits;
				...
				/* memory, register add is 16 clocks + EA, according to manual */
				clocks += 16 + temp_ea_clocks;
				printf("Clocks +%u = %u | ", (16 + temp_ea_clocks), clocks);
			}
			else
			{
				printf("\e[1;33m[WARNING] (execute): first operand is not a register or memory"
					"\e[0;37m\n");
				return;
			}
			return;
		...
		if(op_two_type == Operand_Register)
		{
			...
			print_reg(reg_two);
			printf(" ; ");
			/* register, register add is 3 clocks, according to manual */
			clocks += 3;
			printf("Clocks +3 = %u | ", clocks);
		...
		else if(op_two_type == Operand_Immediate)
		{
			...
			print_reg(reg);
			printf(", %d ; ", imm);
			/* register, immediate add is 4 clocks, according to manual */
			clocks += 4;
			printf("Clocks +4 = %u | ", clocks);
			print_reg(reg);
			printf(":0x%x->0x%x ", temp, registers[reg - 1]);
...
int main(void)
{
	...
	ASSERT(stat("listing_0056_estimating_cycles", &disassembly_stat) == 0);

	unsigned long long disassembly_size = disassembly_stat.st_size;
	s32 disassembly_fd = open("listing_0056_estimating_cycles", O_RDONLY);
