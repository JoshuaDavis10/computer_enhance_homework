...
/* TODO: fairly naive implementation. I think there's better/faster ways to do it with 
just bit manipulation or smn. note this doesn't account for NaN, infinity, 0, etc...
but those basically won't happen for my current use case (computer enhance haversine input
json generator thing) - josh
*/
static jstring jstring_create_double(f64 number, u32 precision)
{
	...
	if(precision != 0)
	{
		jstring_concatenate_raw(&integer_part_string, ".");
		u32 tmp = 0;
		while(tmp < precision)
		{
			fractional_part *= 10;
	...
	{
		jstring_concatenate_raw(&integer_part_string, "0");
	}
	if(!jstring_concatenate_jstring(&integer_part_string, fractional_part_string))
	{
		JSTRING_ASSERT(0, " ");
